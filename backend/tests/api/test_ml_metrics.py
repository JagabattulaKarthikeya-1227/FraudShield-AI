...
def test_get_model_registry_empty(client, test_db, mocker):
    ...
    access_token = create_access_token(identity="admin", additional_claims={"role": "Administrator"})
    response = client.get(
        "/api/v1/ml/registry",
...
def test_get_experiments_empty(client, test_db):
...
def test_get_drift_empty(client, test_db):
