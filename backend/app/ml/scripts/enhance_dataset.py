...
def enhance_dataset(target_majority_size=50000, target_minority_size=50000):
    ...
    if not os.path.exists(input_path):
        raise FileNotFoundError(
            f"Could not find {input_path}. Please make sure the file is in the right location."
        )
    ...
    print(
        "Applying Undersampling on the majority class and SMOTE on the minority class..."
    )
    ...
    under = RandomUnderSampler(
        sampling_strategy={0: target_majority_size}, random_state=42
    )
