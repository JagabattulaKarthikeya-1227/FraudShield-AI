...
def get_stats():
    ...
    if role == "Customer":
        ...
        flagged_tx = Transaction.query.filter_by(user_id=user.id, status=TransactionStatus.FLAGGED).count()
        ...
        queue_size = Transaction.query.filter_by(status=TransactionStatus.FLAGGED).count()
        my_reviews = db.session.query(db.func.count()).filter(
            Transaction.review.has(analyst_id=user.id)
        ).scalar() or 0
        return success_response(data={
