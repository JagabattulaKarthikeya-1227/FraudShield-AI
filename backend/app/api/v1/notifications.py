...
def get_notifications():
    ...
    if user.role.value in ['Administrator', 'Fraud Analyst']:
        notifs = [
            {"id": "n1", "title": "High Risk Alert", "message": "Transaction TX-9428 requires immediate review.", "type": "alert", "priority": "high", "read": False, "timestamp": datetime.datetime.utcnow().isoformat()},
            {"id": "n2", "title": "Model Deployed", "message": "Hybrid Ensemble v1.4 successfully rolled out.", "type": "system", "priority": "normal", "read": True, "timestamp": (datetime.datetime.utcnow() - datetime.timedelta(hours=2)).isoformat()}
        ]
    else:
        notifs = [
            {"id": "n3", "title": "Welcome", "message": "Welcome to FraudShield AI.", "type": "system", "priority": "normal", "read": False, "timestamp": datetime.datetime.utcnow().isoformat()}
        ]
