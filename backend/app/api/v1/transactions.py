...
def review_transaction(tx_id):
    ...
    if action == 'approve':
        ...
        tx.status = "approved" # False positive by ML, approved by human
    elif action == 'reject':
        review.status = ReviewStatus.REJECTED
        tx.status = "declined" # True positive, fraud confirmed
    else:
...
def get_heatmap():
    ...
    ).filter(Transaction.status == 'declined').group_by('dow', 'hour')
    ...
    if not results:
        ...
        for i in range(7):
            for j in range(24):
                ...
                if j < 6 or j > 22: base += random.random() * 10
                echarts_data.append([j, i, int(base)])
    else:
        for dow, hour, count in results:
            if dow == 7: echarts_day = 0
            elif dow == 6: echarts_day = 1
            elif dow == 5: echarts_day = 2
            elif dow == 4: echarts_day = 3
            elif dow == 3: echarts_day = 4
            elif dow == 2: echarts_day = 5
            elif dow == 1: echarts_day = 6
            else: echarts_day = 6
